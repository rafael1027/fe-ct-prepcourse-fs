...
function promedioResultadosTest(resultadosTest) {
   ...
   // Tu código:
   var suma = 0;
  var cantidadElementos = resultadosTest.length;
...
function mesesDelAño(array) {
   ...
   array.forEach((mes)=>{ if(mesesBuscados.includes(mes) && !mesesEncontrados.includes(mes)){
      mesesEncontrados.push(mes)
   }})

   if(mesesEncontrados.length === mesesBuscados.length){
      return "todos los meses se encontraron " 
   }else{
...
function mayorACien(array) {
   ...
   // Tu código:
   var mayoresAcien=[];
   array.forEach((num)=>{if (num > 100 && num <= 200){
